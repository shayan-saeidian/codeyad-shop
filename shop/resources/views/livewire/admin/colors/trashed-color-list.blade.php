<div class="grid grid-cols-1 gap-6 p-4">
    <div class="panel p-5">
        <a href="{{route('admin.colors.list')}}" type="button" class="btn btn-danger mb-4 w-[300px]">
            ...
            <div class="table-responsive">
                <table>
                    ...
                    <tr>
                        ...
                        <th class="text-center">نمونه رنگ</th>
                        <th class="text-center">کد رنگ</th>
                        <th class="text-center">تاریخ رنگ</th>
                    ...
                    @foreach($this->colors as $index => $color)
                        <tr>
                            <td >{{$this->colors->firstItem()+$index}}</td>
                            <td class="whitespace-nowrap">{{$color->name}}</td>
                            <td>
                                <span class="inline-block w-6 h-6 rounded-full border" style="background-color: {{ $color->code }}"></span>
                            </td>
                            <td class="whitespace-nowrap">{{$color->code}}</td>
                            <td class="whitespace-nowrap">{{Verta::instance($color->created_at)->formatJalaliDate()}}</td>
                            <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                <button wire:click="restoreRow({{$color->id}})" type="button" x-tooltip="بازآوری">
                                ...
                                <button
                                    wire:click="$dispatch('hard-delete-color', { color_id: {{ $color->id }} })"
                                    type="button"
                                    x-tooltip="حذف دایم"
                                >
...
@script
<script>
    Livewire.on('hard-delete-color', (event) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Livewire.dispatch('hard-destroy-color', { color_id : event.color_id })
                Swal.fire({
                    title: "حذف انجام شد!",
                    icon: "success"
                });
