<div class="grid grid-cols-1 gap-6 p-4">
    <div class="panel p-5">
        <a href="{{route('admin.guaranties.list')}}" type="button" class="btn btn-danger mb-4 w-[300px]">
            ...
            <div class="table-responsive">
                <table>
                    ...
                    @foreach($this->guaranties as $index => $guaranty)
                        <tr>
                            <td >{{$this->guaranties->firstItem()+$index}}</td>
                            <td class="whitespace-nowrap">{{$guaranty->name}}</td>
                            <td class="whitespace-nowrap">{{Verta::instance($guaranty->created_at)->formatJalaliDate()}}</td>
                            <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                <button wire:click="restoreRow({{$guaranty->id}})" type="button" x-tooltip="بازآوری">
                                ...
                                <button
                                    wire:click="$dispatch('hard-delete-guaranty', { guaranty_id: {{ $guaranty->id }} })"
                                    type="button"
                                    x-tooltip="حذف دایم"
                                >
...
@script
<script>
    Livewire.on('hard-delete-guaranty', (event) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Livewire.dispatch('hard-destroy-guaranty', { guaranty_id : event.guaranty_id })
                Swal.fire({
                    title: "حذف انجام شد!",
                    icon: "success"
                });
